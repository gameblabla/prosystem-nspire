...
void initSDL(void) 
{
	...
	video = SDL_SetVideoMode(320, 240, vidbpp, SDL_SWSURFACE );
	if(video == NULL) {
		fprintf(stderr, "Couldn't set video mode: %s\n", SDL_GetError());
		exit(1);
	}
...
int main(int argc, char *argv[]) {
	...
	sprintf(current_conf_app,"./prosystem.cfg.tns");
	initSDL();
	sound_InitSDL();
	...
	while (m_Flag != GF_GAMEQUIT) {
		...
		switch (m_Flag) {
			...
			case GF_GAMERUNNING:
				...
				if (keys[SDLK_LSHIFT] == SDL_PRESSED) { tchepres(5); }  // BUTTON #2
				if (keys[SDLK_SPACE] == SDL_PRESSED) { tchepres(6); }  // KEYPAD #3
				if (keys[SDLK_MENU] == SDL_PRESSED) { tchepres(7); }  // KEYPAD #4
